...
static int	inner_b_loop(t_boundary *b, t_game *game)
{
	...
	assign_min_max(b);
	radius_y = b->min_y;
	while (radius_y <= b->max_y) // For each row in player radius box
	{
		radius_x = b->min_x;
		while (radius_x <= b->max_x) // For each column in that  box
		{
			if (get_map_point(radius_y, radius_x, game) == 1) // Check if that tile is a wall (1)
			{
...
int	boundary_check(t_vector *new, t_game *game) //TODO: check if any point on line next step = wall hit, not only end point (=going thru walls rn)
{
	...
	b.player_radius = PLAYER_RADIUS; //acts as buffer around player to not collide with walls
